...
public class TestSelect extends SparkCatalogTestBase {
  ...
  public void testInvalidTimeTravelBasedOnBothAsOfAndTableIdentifier() {
    ...
    assertThatThrownBy(
        ...
        .as("Cannot do time-travel based on both table identifier and AS OF")
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("Cannot do time-travel based on both table identifier and AS OF");
  ...
  public void testSpecifySnapshotAndTimestamp() {
    ...
    assertThatThrownBy(
        ...
        .as("Should not be able to specify both snapshot id and timestamp")
        .isInstanceOf(IllegalArgumentException.class)
  ...
  public void testTimestampAsOfWithAlterColumn() {
    ...
    // query with old schema specification
    List<Object[]> actualWithLongFormat =
  ...
  public void testBranchReferenceWithAlterColumn() {
    ...
    // query with old schema specification
    List<Object[]> actual1 =
        sql("SELECT * FROM %s VERSION AS OF 'test_branch' WHERE id = 1", tableName);
    assertEquals("Snapshot at specific branch reference name", expected, actual1);
    ...
    if (!"spark_catalog".equals(catalogName)) {
      // query with old schema specification
      List<Object[]> actual2 =
          sql("SELECT * FROM %s.branch_test_branch WHERE id = 1", tableName);
  ...
  public void testTagReferenceWithAlterColumn() {
    ...
    if (!"spark_catalog".equals(catalogName)) {
      // query with old schema specification
      List<Object[]> actual2 =
          sql("SELECT * FROM %s.tag_test_tag WHERE id = 1", tableName);
